...
public sealed class RegistryAppSettingsBrandingTests
{
    ...
    {
        using var key = Registry.CurrentUser.CreateSubKey(settingsPath, writable: true);
        key?.SetValue(name, value, RegistryValueKind.DWord);
    }

    private static int? ReadDword(string settingsPath, string name)
    {
        using var key = Registry.CurrentUser.OpenSubKey(settingsPath, writable: false);
    ...
    private static void DeleteValue(string settingsPath, string name)
    {
        try
        {
            using var key = Registry.CurrentUser.OpenSubKey(settingsPath, writable: true);
    ...
    private static void RemoveKeyIfEmpty(string settingsPath)
    {
        ...
        try
        {
            Registry.CurrentUser.DeleteSubKey(settingsPath, throwOnMissingSubKey: false);
        }
        catch { }
    }
}
